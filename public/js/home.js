document.addEventListener('DOMContentLoaded', function() {
    ...
    if (carousel) {
        ...
        if (total > 0) {
            ...
            // Make orbit radius responsive based on screen size
            function getResponsiveOrbitRadius() {
                const screenWidth = window.innerWidth;
                
                if (screenWidth <= 576) {
                    return 150;   // Very small screens
                } else if (screenWidth <= 768) {
                    return 180;  // Mobile screens
                } else if (screenWidth <= 991) {
                    return 210;  // Tablet screens
                } else if (screenWidth <= 1200) {
                    return 230;  // Small desktop
                } else {
                    return 250;  // Large screens (your current value)
                }
            }

            const BASE_ORBIT_RADIUS = getResponsiveOrbitRadius();
